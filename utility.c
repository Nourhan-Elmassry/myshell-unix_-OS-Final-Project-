...
/* Function "Execute" : used as the function system(), it interpret the input, call function "Command_exec" to execute all the commands */
int Execute (char *buffer) {
	...
	if (error || args[0] == NULL)	// If the input format error	or  if there's anything there
		return -1;
...
int Command_exec (char **args, const Redirect *Inputs, const Redirect *Outputs, int *states) {
	...
	FILE *inputfile = NULL;
	int saved_stdout = -1; /* for restoring stdout after redirection */
	pid_t newpid;
	int flag;

	/* Handle batch invocation: myshell <file> or myshell file */
	...
	/* stdout redirection (must also work for internal commands) */
	if (states[2]) {
		get_fullpath(filepath, Outputs->filename);
		saved_stdout = dup(STDOUT_FILENO);
		if (saved_stdout < 0) {
			Error(-9, NULL, NULL, states, "dup");
			if (states[0])
				exit(1);
			return -1;
		}
		...
		switch (newpid = fork()) {
			...
			case 0:
				if (states[1]) {
					...
					inputfile = freopen(filepath, "r", stdin);
					if (inputfile == NULL) {
	...
	/* handle multiple output redirections by re-executing with next output */
	if (states[2] > 1) {
		states[2]--;
...
int Error (int errortype, char **args, const Redirect *IOputs, const int *states, char *msg) {
	...
	switch (errortype) {
		...
		case  4:
			...
			fprintf(stderr, "\nType 'help %s' to get usage about '%s'.\n", msg, msg);
			break;
		case  5:
...
int Command_strtok (char *buf, char **args, int *states, Redirect *Inputs, Redirect *Outputs) {	// redirection of InPut & OutPuts
	...
	char c;
	states[0] = states[1] = states[2] = states[3] = states[4] = 0;	// states[0] is the symbol of background process;
										// states[1] number of input redirection;
										// states[2] the number of output redirection;
										// states[3] number of parameters;
										// states[4] label input redirection before keybroad input;
	...
	while (buf[++i] && buf[i] != '#') {	// scan the command, "#" for remark
		...
		switch (c) {	// Character Analysis
			...
			default:
				...
				if (c == '\\' && buf[i+1] == ' ') {	// Escape chars e.g. "a\ b" means "a b" in filename or directoryname
					buf[i] = ' ';
					if (!isspace(buf[i+2])) {
						j = i+1;
						while (buf[++j])
...
void Command_clear (void) {	// system("clear");
	pid_t newpid;
	switch (newpid = fork()) {
		case -1:
			Error(-9, NULL, NULL, NULL, "fork");
...
int Command_dir (char **args, const Redirect *Inputs, int *states) {
	...
	if (args[1]) {	// the argument of pathname is given
		...
		get_fullpath(dirpath, args[1]);
       	...
       	strcpy(dirpath, ".");	// just   "dir"   means   " dir . "
...
int Command_echo (char **args, const Redirect *Inputs, int *states) {
	...
	if (states[4]) {	// input redirection is before args[1],   e.g.     echo  < a.txt  hello
		...
		for (j = 0; j < states[1]; j++) {	// e.g.  echo < a.txt < b.txt
			...
			while (fgets(buf, MAX_BUFFER, inputfile) != NULL) {	// display the contents of file
...
void Command_delay (int n) {
	/* Used only to reduce output interleaving for background jobs.
	 * Use nanosleep to stay POSIX.1-2008 compliant (usleep is obsolete).
	 */
	if (n <= 0)
		return;
	struct timespec ts;
	ts.tv_sec = 0;
	ts.tv_nsec = (long)n * 1000000L;
...
int Command_help (char **args, const Redirect *Outputs, int *states) {
	...
	readme = fopen(getenv("readme_path"), "r"); // readme_path is exported by myshell.c
	if (readme == NULL) {
		Error(-6, NULL, NULL, NULL, getenv("readme_path"));
		return -2;
	}
...
int Command_bat (char **args, const Redirect *Inputs, const Redirect *Outputs, int *states) {	// fprintf(stderr, "mybat");
	FILE *inputfile;
	char filepath[MAX_PATH];
	int i = 0;
	...
	if (args[1]) {	// e.g.,  myshell test.bat    myshell < a.bxt
		...
		fprintf(stdout, "\nMade by: Nourhan Muneer Elmassry\n");
	return 0;
}
